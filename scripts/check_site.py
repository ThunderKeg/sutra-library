...
def main() -> int:
    ...
    require("20260723-audio-prompt-copy-v1" in app_script, "app version was not bumped for the audio prompt copy update")
    require("sutra-library-v20-20260723-audio-prompt-copy" in service_worker, "service worker cache version was not bumped")
    ...
    require(all(label in reader_html for label in ("從這裡播放", "回到播放處")), "playing-scroll audio choices are incomplete")
    require(all(label not in reader_html for label in ("從這裡重聽", "繼續閱讀", "回到播放位置")), "legacy audio choices must be removed")
