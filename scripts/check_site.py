...
def main() -> int:
    ...
    require("20260723-unified-reading-audio-v1" in app_script, "app version was not bumped for unified reading/audio progress")
    require("sutra-library-v19-20260723-unified-reading-audio" in service_worker, "service worker cache version was not bumped")
    require('id="audioPanel"' not in reader_html and 'id="readerAudio"' in reader_html, "audio must use the direct header control without a player panel")
    require('preload="none"' in reader_html, "reader audio must not load before the user starts playback")
    require('<audio id="readerAudio" preload="none" hidden></audio>' in reader_html, "native audio progress bar must stay hidden")
    require("crossorigin" not in reader_html, "cross-origin media playback must not require CORS credentials")
    ...
    require("discardLegacyAudioProgress" in reader_script and "restoreAudioProgress" not in reader_script and "saveAudioProgress" not in reader_script, "reading position must be the only saved progress")
    require("ensureAudioSource" in reader_script and "readerAudio.src = currentAudioTrack.url" in reader_script, "audio source must attach only after direct user playback")
    require("startAudioFromReadingPosition" in reader_script and 'audioButton.addEventListener("click", toggleAudioPlayback)' in reader_script, "header audio button must play directly from the reading position")
    require('id="audioFollowPrompt"' in reader_html and 'id="audioReplayHereButton"' in reader_html, "contextual replay controls missing")
    require(all(label in reader_html for label in ("從這裡重聽", "回到播放處")), "playing-scroll audio choices are incomplete")
    require(all(label not in reader_html for label in ("從這裡播放", "繼續閱讀", "回到播放位置")), "legacy audio choices must be removed")
    require("audioAutoFollowToggle" not in reader_html and "audioSeekOnScrollToggle" not in reader_html and "audioResumeFollowToggle" not in reader_html, "audio-follow preference switches must be removed")
    ...
    require("seekAudioToVisibleText" in reader_script and "finishManualScroll" in reader_script and "replayFromVisibleText" in reader_script, "contextual text-to-audio linkage missing")
